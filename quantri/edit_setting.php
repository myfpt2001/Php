...
if (isset($_POST['btn'])) {
    extract($_REQUEST);
    if ($logo == '' || $address == '' || $phone == '' || $information == '' || $Social_Network == '') {
        $thongbao = "Bạn  đang bỏ trống 1 trong các trường trên";
    } else {
        if (($_FILES['logo']['size'] > 0)) {
            $logo = $_FILES['logo']['name'];
        }
        $sql = "UPDATE setting SET logo = '$logo', address = '$address', phone = '$phone', information = '$information', Social_Network = '$Social_Network'
            ...
            <div class="col-xl-10 content_right pl-0 pr-0">
                <div>
                    ...
                    <div class="menu_main_right ml-3">
                        <h2 class="title_edit">Sửa Setting</h2>
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="setting_id" value="<?= $result['setting_id'] ?>">
                            <label for="" class="name">Địa Chỉ:</label>&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;
                            <input type="text" name="address" id="ten" value="<?= $result['address'] ?>">
                            <p></p>
                            <label for="" class="name">Hình ảnh</label>&nbsp;&nbsp;&nbsp;&nbsp;&emsp;
                            <input type="file" id="file" name="logo">
                            <?php if (!empty($result['logo'])) : ?>
                            <input type="hidden" name="logo" value="<?= $result['logo'] ?>">
                            <br>
                            <img src="../image/header/<?= $result['logo'] ?>" width="200px" alt="" class="img">
                            <?php endif; ?>
                            <p></p>
                            <label for="" class="name">Thông Tin:</label>
                            <input type="text" name="information" id="ten" value="<?= $result['information'] ?>">
                            <p></p>
                            <label for="" class="name">Điện Thoại:</label>
                            <input type="text" name="phone" id="ten" value="<?= $result['phone'] ?>">
                            <p></p>
                            <label for="" class="name">Các mạng Xã Hội</label>
                            <input type="text" name="Social_Network" id="ten" value="<?= $result['Social_Network'] ?>">
                            <p></p>
